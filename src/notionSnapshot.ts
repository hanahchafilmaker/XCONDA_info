 ...
 * 서버(CORS 없음)에서 읽어, 사이트와 같은 경로에 `notion-content.json` 으로
 * 커밋합니다. 사이트는 같은 출처의 정적 파일만 읽으면 되므로
 *   - CORS 차단
 *   - 무료 공개 프록시(notion-api.splitbee.io) 장애
 * 와 무관하게 항상 콘텐츠를 표시할 수 있습니다.
 *
...
/** 스냅샷을 읽습니다. 없거나 오류면 null (공개 API 경로로 이어서 진행) */
export function loadSnapshot(force = false): Promise<NotionSnapshot | null> {
  ...
  pending = (async () => {
    try {
      const res = await fetch(snapshotUrl(), { cache: "no-cache", headers: { Accept: "application/json" } });
...
/** 캐시 무효화 — 수동 동기화(🔄) 시 최신 스냅샷을 다시 받도록 합니다. */
