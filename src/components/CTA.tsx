          ...
          <Reveal className="h-full">
            <div className="relative isolate flex h-full flex-col justify-between overflow-hidden rounded-[32px] p-8 ring-1 ring-white/10 sm:p-12">
                ...
                <h2 className="mt-4 text-balance text-3xl font-bold leading-[1.2] tracking-[-0.035em] text-white sm:text-5xl">
                  {lang === "ko" ? (
                    ...
                    <>
                      Enter a scenario —
                      <br />
                      nine cuts that become <Serif className="text-gradient font-normal">one story</Serif>.
                    </>
                  )}
                </h2>
                <p className="mt-4 max-w-md text-zinc-300">{t("cta.subtitle")}</p>
              </div>
              <div className="mt-10 flex flex-wrap gap-3">
                ...
                >
                  {t("common.openStudioTop")} <ArrowUpRight className="h-4 w-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
                </a>
          ...
          <div className="grid gap-4">
            ...
            <Reveal delay={100}>
              <div className="glass rounded-[28px] p-7">
                ...
                <h3 className="mt-4 text-lg font-semibold text-white">{t("cta.subscribe.title")}</h3>
                <p className="mt-1 text-sm text-zinc-400">{t("cta.subscribe.desc")}</p>
                <form onSubmit={onSubmit} noValidate className="mt-5">
                  <div className={cn("flex gap-2 rounded-full bg-black/30 p-1 ring-1 ring-inset ring-white/10", state === "error" && "ring-rose-400/60")}>
                    <label htmlFor="sub-email" className="sr-only">{t("cta.subscribe.emailLabel")}</label>
                    <input
                      id="sub-email"
                      type="email"
                      autoComplete="email"
                    ...
                    >
                      {state === "loading" ? <span className="h-4 w-4 animate-spin rounded-full border-2 border-ink-950/20 border-t-ink-950" /> : state === "done" ? <><Check className="h-4 w-4" /> {t("cta.subscribe.done")}</> : <>{t("cta.subscribe.submit")} <ArrowRight className="h-3.5 w-3.5" /></>}
                    </button>
                  </div>
                  <p id="sub-help" aria-live="polite" className={cn("mt-2 px-2 text-xs", state === "error" ? "text-rose-300" : "text-zinc-500")}>
                    {state === "error" ? t("cta.subscribe.errHelp") : state === "done" ? t("cta.subscribe.doneHelp") : t("cta.subscribe.idleHelp")}
                  </p>
                </form>
              </div>
            </Reveal>
            ...
            <Reveal delay={180}>
              <div className="glass flex flex-col gap-4 rounded-[28px] p-7">
                <h3 className="text-lg font-semibold text-white">{t("cta.contact.title")}</h3>
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                  <a href="mailto:support@xconda.ai" className="group flex items-center gap-3 rounded-2xl bg-white/[0.03] p-4 ring-1 ring-inset ring-white/[0.06] transition hover:bg-white/[0.07]">
                    ...
                    <span className="text-sm"><span className="block font-medium text-white">{t("cta.contact.email")}</span><span className="text-xs text-zinc-500">support@xconda.ai</span></span>
                  </a>
                  <a href={STUDIO_URL} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-3 rounded-2xl bg-white/[0.03] p-4 ring-1 ring-inset ring-white/[0.06] transition hover:bg-white/[0.07]">
                    ...
                    <span className="text-sm"><span className="block font-medium text-white">{t("cta.contact.studio")}</span><span className="text-xs text-zinc-500">{t("cta.contact.studioSub")}</span></span>
                  </a>
                </div>
              </div>
            </Reveal>
